  ...
  ({                                                                        \
    _OCMSilenceWarnings(                                                    \
        ...
        @try { invocation; } @catch (...) {                                 \
          ...
          @throw;                                                           \
        } @finally { recorder = [OCMMacroState endStubMacro]; } recorder;); \
  })
...
#define OCMVerifyAll(mock) \
  [(OCMockObject *)mock verifyAtLocation:OCMMakeLocation(self, __FILE__, __LINE__)]

#define OCMVerifyAllWithDelay(mock, delay) \
  [(OCMockObject *)mock verifyWithDelay:delay atLocation:OCMMakeLocation(self, __FILE__, __LINE__)]
  ...
  ({                                                                                          \
    _OCMSilenceWarnings(                                                                      \
        ...
        @try { invocation; } @catch (...) {                                                   \
          ...
          /* NOLINTNEXTLINE(google-objc-avoid-throwing-exception) */                          \
          @throw;                                                                             \
        } @finally { [OCMMacroState endVerifyMacro]; });                                      \
  })
...
// explanation for macros below here:
...
#define OCMVerify(...) \
  _OCMVerify_X(, ##__VA_ARGS__, _OCMVerify_2(__VA_ARGS__), _OCMVerify_1(__VA_ARGS__))
